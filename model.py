...
def generate_batch_from_data(data, size = 64):
    images = np.zeros((size, *preprocessed.shape))
    steering_angles = np.zeros(size)
    while 1:
        for i in range(size):
            row_num = np.random.randint(len(data))
            images[i] = preprocess(data['center'][row_num])
            steering_angles[i] = data['steering'][row_num]
...
data_train, data_val = test_train_split(driving_log)
